...
pss =[0 for i in range(24)]                                 # [0,0,0....23개] 1차원리스트 생성 ★★★ list 를 만듬과 동시에 초기화 [리스트내포 라함]
for row in data:
    if row[2] == d_code:
        pss[row[1]] += row[3]                               # row[1]시간int 0에서23까지 / row[3]인구int value / +=row[3] 최초값 0에 누적합
pss =[row[3]/31 for row[3] in pss]
